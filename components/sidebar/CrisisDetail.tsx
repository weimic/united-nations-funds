...
import { getSeverityBadgeColor, getNeglectColor } from "@/components/shared/severity-colors";
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-[10px] font-bold font-mono ${pct < 50 ? "text-red-400" : "text-cyan-400"}`}
              >
                {pct.toFixed(1)}%
              </span>
            </div>
              ...
              {crisis.countries.map((entry, idx) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono text-cyan-400/50 w-5 shrink-0">
                      {idx + 1}.
                    </span>
                    ...
                    <Badge
                      variant="outline"
                      className={`text-[9px] px-1.5 py-0 font-mono ${getSeverityBadgeColor(entry.severityCategory)}`}
                    >
                      {entry.severityIndex.toFixed(1)}
                    </Badge>
                    {entry.overallFunding && (
                      <span
                        className={`text-[10px] font-mono ${entry.overallFunding.percentFunded < 50 ? "text-red-400" : "text-cyan-400"}`}
                      >
                  ...
                  {entry.neglectIndex > 0 && (
                    <div className="mt-1.5 flex items-center gap-2">
                      <div className="flex-1 h-1 rounded-full bg-muted/20 overflow-hidden">
                        <div
                          className={`h-full rounded-full ${getNeglectColor(entry.neglectIndex)}`}
                          style={{ width: `${(entry.neglectIndex / 8) * 100}%` }}
                        />
                      </div>
